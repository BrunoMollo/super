...
export const t_user = pgTable(
	...
	(t) => ({
		username_index: uniqueIndex('username_idx').on(t.username)
	})
);
...
export const t_product = pgTable(
	...
	(t) => ({
		bar_code_index: uniqueIndex('bar_code_idx').on(t.bar_code)
	})
);
...
export const t_client = pgTable(
	...
	(t) => ({
		dni_index: uniqueIndex('dni_idx').on(t.dni)
	})
);
