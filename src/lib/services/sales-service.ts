...
export async function get_products(input: { product_id: number; quantity: number }[]) {
	...
	for (const { product_id, quantity } of input) {
		const { desc, price, iva_percentage } = result.find((x) => x.id === product_id)!;
		products.push({
			name: desc,
			product_id,
			quantity,
...
export async function register_sale(
	...
	await sale_repo.register_sale({
		seller_id,
		products,
		client_id,
		bill: { cae, expiration_date_of_cae, voucher_number: billNumber }
	});
	return { ok: true };
}
